# Docstring, PHPDoc, and JSDoc Rules
...
- Internal helpers: only when the contract is not expressible in the signature (units, invariants, side effects, "why"). An internal one-liner with an intention-revealing name usually needs nothing — and clean-code-pro's comment rules apply.
